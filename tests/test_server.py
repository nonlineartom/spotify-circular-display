...
def remote(address="192.168.24.40"):
...
def test_reverse_proxy_markers_disqualify_implicit_loopback_owner(client):
    ...
    proxied = {
        ...
        "HTTP_X_FORWARDED_FOR": "192.168.24.40",
...
@pytest.mark.parametrize("device,error_fragment", [
    ({"host": "192.168.24.67", "pixel_count": "oops"}, "integer"),
    ({"host": "192.168.24.67", "pixel_count": 2049}, "between"),
...
def test_wled_discovery_is_owner_scoped_and_requests_scan(client, monkeypatch):
    ...
    server._wled_record_device("Record halo", "192.168.24.67", 80, 120)
...
def test_wled_discovery_cache_retains_then_evicts_by_ttl(client, monkeypatch):
    ...
    assert server._wled_active_devices()[0]["ip"] == "192.168.24.67"
    now[0] += 2
...
def test_wled_scan_batch_caps_total_probe_work(client, monkeypatch):
    ...
    config_path.write_text(json.dumps(config))
    monkeypatch.setattr(server, "get_local_ip", lambda: "192.168.24.10")
    monkeypatch.setattr(
        server,
        "_local_scan_network",
        lambda: server.ipaddress.ip_network("192.168.24.0/22"),
    )
    probed = []
    ...
    assert network == server.ipaddress.ip_network("192.168.24.0/22")
...
def test_unlinked_receiver_gets_generic_crate_without_user_api_calls(client, monkeypatch):
    _web, _ = client
    monkeypatch.setattr(server, "load_vinyls", lambda *_args: [{
        "id": "fixture-album", "title": "Fixture Album",
        "uri": "spotify:album:fixture", "available": True,
    }])
    ...
    assert payload["profile_state"] == "unlinked"
    assert [section["id"] for section in payload["sections"]] == ["vinyls", "house"]
