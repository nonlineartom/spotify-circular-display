...
| OP-07 | Units hard-coded user, path, UID, socket and fixed sleeps | Setup renders the actual user/path/port; system and graphical-user units use readiness/restart policies and tighter service hardening. The kiosk waits for the web-readiness endpoint rather than receiver health, so an offline receiver cannot prevent the error UI from booting. |
